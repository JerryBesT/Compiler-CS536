...
// Title:            P5
// Files:            ast.java cdull.cup cdull.jlex
//                   DuplicateSymException.javaEmpytSymTableException.java
//                   WrongArgumentException.java ErrMsg.javaSym.java SymTable.java Type.java
//                   MakefileP5.java
...
class DeclListNode extends ASTnode {
    ...
    // it will only check the function declaration
    public void typeCheck() {
...
class FnBodyNode extends ASTnode {
    ...
    public void typeCheck(Type ret) {
        declList.typeCheck();
        stmtList.typeCheck(ret);
    }
...
class ExpListNode extends ASTnode {
    ...
    public void typeCheck(IdNode id) {
        FnSym func = (FnSym)id.sym();
        List<Type> formals = func.getParamTypes();
        for(int i = 0;i < exps.size();i++) {
            Type a = exps.get(i).typeCheck();
            Type f = formals.get(i);
            if(a.isErrorType())
                continue;
            if(!(a.equals(f))) {
                ErrMsg.fatal(exps.get(i).lineNum(), exps.get(i).charNum(), "Type of actual does not match type of formal");
            }
        } 
    }
...
class FnDeclNode extends DeclNode {
    ...
    public void typeCheck() {
        formalsList.typeCheck();
        FnSym func = (FnSym)id.sym();
        fnBody.typeCheck(func.getReturnType()); 
    }
...
abstract class StmtNode extends ASTnode {
    ...
    abstract public void typeCheck(Type ret);
}
...
class ReadStmtNode extends StmtNode {
    ...
    public void typeCheck(Type ret) {
        Type t = exp.typeCheck();
        if(t.isFnType()) {
            ErrMsg.fatal(exp.lineNum(), exp.charNum(), "Attempt to read a function");
            return;
        }
        if(t.isStructType()) {
            ErrMsg.fatal(exp.lineNum(), exp.charNum(), "Attempt to read a struct variable");
            return;
        }
        if(t.isStructDefType()) {
            ErrMsg.fatal(exp.lineNum(), exp.charNum(), "Attempt to read a struct name");
            return;
        }
    }
...
class WriteStmtNode extends StmtNode {
    ...
    public void typeCheck(Type ret) {
        Type t = exp.typeCheck();
        if(t.isFnType()) {
            ErrMsg.fatal(exp.lineNum(), exp.charNum(), "Attempt to write a function");
            return;
        }
        if(t.isStructType()) {
            ErrMsg.fatal(exp.lineNum(), exp.charNum(), "Attempt to write a struct variable");
            return;
        }
        if(t.isStructDefType()) {
            ErrMsg.fatal(exp.lineNum(), exp.charNum(), "Attempt to write a struct name");
            return;
        }
        if(t.isVoidType()) {
            ErrMsg.fatal(exp.lineNum(), exp.charNum(), "Attempt to write void");
            return;
        }
    }
    // 1 kid
...
class IfStmtNode extends StmtNode {
    ...
    public void typeCheck(Type ret) {
        Type t = exp.typeCheck();
        if(!(t instanceof ErrorType) && !(t instanceof BoolType)) {
            ErrMsg.fatal(exp.lineNum(), exp.charNum(), 
...
class IfElseStmtNode extends StmtNode {
    ...
    public void typeCheck(Type ret) {
        ...
        thenStmtList.typeCheck(ret);
        elseStmtList.typeCheck(ret);
        return;
    }
    // 5 kids
    private ExpNode exp;
...
class ReturnStmtNode extends StmtNode {
    ...
    public void typeCheck(Type ret) {
        if(exp == null && !(ret.isVoidType()))
            ErrMsg.fatal(0, 0, "Missing return value"); 
        if(exp != null) {
            Type fuck = exp.typeCheck();
            if(ret.isVoidType())
                ErrMsg.fatal(exp.lineNum(), exp.charNum(), "Return with a value in a void function"); 
            else if(!(fuck.equals(ret)))
                ErrMsg.fatal(exp.lineNum(), exp.charNum(), "Bad return value"); 
        }
...
class AssignNode extends ExpNode {
    ...
    public Type typeCheck() {
        Type leftType = lhs.typeCheck();
        Type rightType = exp.typeCheck();
        if(leftType.isFnType() && rightType.isFnType()) {
            ErrMsg.fatal(lhs.lineNum(), lhs.charNum(), "Function assignment");
            return new ErrorType();
        }
        if(leftType.isStructType() && rightType.isStructType()) {
            ErrMsg.fatal(lhs.lineNum(), lhs.charNum(), "Struct variable assignment");
            return new ErrorType();
        }
        if(leftType.isStructDefType() && rightType.isStructDefType()) {
            ErrMsg.fatal(lhs.lineNum(), lhs.charNum(), "Struct name assignment");
            return new ErrorType();
        }
        if(rightType instanceof ErrorType || (leftType instanceof ErrorType))
            return new ErrorType();

        if(!(rightType instanceof ErrorType) && !(leftType.equals(rightType))) {
            ErrMsg.fatal(lhs.lineNum(), lhs.charNum(), "Type mismatch");
            return new ErrorType();
        }
...
class PlusNode extends BinaryExpNode {
    ...
    public Type typeCheck() {
        ...
        if(!(t1 instanceof ErrorType) && !(t1 instanceof IntType)) {
        ...
        if(!(t2 instanceof ErrorType) && !(t2 instanceof IntType)) {
...
class AndNode extends BinaryExpNode {
    ...
    public Type typeCheck() {
        ...
        if(!(t1 instanceof ErrorType) &&!(t1 instanceof BoolType)) {
            ErrMsg.fatal(exp1.lineNum(), exp1.charNum(), 
                    "Logical operator applied to non-bool operand");
            err = true; 
        }
        ...
        if(!(t2 instanceof ErrorType) && !(t2 instanceof BoolType)) {
            ErrMsg.fatal(exp2.lineNum(), exp2.charNum(), 
                    "Logical operator applied to non-bool operand");
            err = true;
        }
...
class EqualsNode extends BinaryExpNode {
    ...
    public Type typeCheck() {
        ...
        if(leftType.isVoidType() && rightType.isVoidType()) {
            ErrMsg.fatal(exp1.lineNum(), exp1.charNum(), "Equality operator applied to void functions");
            return new ErrorType();
        }
        if(leftType.isFnType() && rightType.isFnType()) {
            ErrMsg.fatal(exp1.lineNum(), exp1.charNum(), "Equality operator applied to functions");
            return new ErrorType();
        }
        if(leftType.isStructType() && rightType.isStructType()) {
            ErrMsg.fatal(exp1.lineNum(), exp1.charNum(), "Equality operator applied to struct variables");
            return new ErrorType();
        }
        if(leftType.isStructDefType() && rightType.isStructDefType()) {
            ErrMsg.fatal(exp1.lineNum(), exp1.charNum(), "Equality operator applied to struct names");
            return new ErrorType();
        }
        ...
        if(!(rightType instanceof ErrorType) && !(leftType.equals(rightType))) {
            ErrMsg.fatal(exp1.lineNum(), exp1.charNum(), "Type mismatch");
            return new ErrorType();
        }
